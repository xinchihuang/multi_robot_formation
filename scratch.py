...
def setVrepHandles(objectNames, clientID, handleNameSuffix=""):
    ...
    res1, robotHandle = vrep.simxGetObjectHandle(
        clientID, handleNames[0] + handleNameSuffix, vrep.simx_opmode_oneshot_wait
    )

    res2, motorLeftHandle = vrep.simxGetObjectHandle(
        clientID, handleNames[1] + handleNameSuffix, vrep.simx_opmode_oneshot_wait
    )
    res3, motorRightHandle = vrep.simxGetObjectHandle(
        clientID, handleNames[2] + handleNameSuffix, vrep.simx_opmode_oneshot_wait
    )
...
def propagate(clientID, motorLeftHandle, motorRightHandle, omega1, omega2):
    ...
    vrep.simxSetJointTargetVelocity(
        clientID, motorLeftHandle, omega1, vrep.simx_opmode_oneshot
    )
    vrep.simxSetJointTargetVelocity(
        clientID, motorRightHandle, omega2, vrep.simx_opmode_oneshot
    )


clientID = initVrep()
motorLeftHandle, motorRightHandle = setVrepHandles(
    objectNames, clientID, handleNameSuffix=""
)
