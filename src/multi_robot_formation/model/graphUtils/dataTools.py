...
from utils import gabreil_graph as graph
        ...
        if len(args) == 1:
            ...
            if type(args[0]) == int:
                ...
                selectedIndices = np.random.choice(
                    nSamples, size=args[0], replace=False
                )
    ...
    def astype(self, dataType):
        ...
        if "torch" in repr(dataType):  # If it is torch
            for key in self.samples.keys():
                self.samples[key]["signals"] = torch.tensor(
                    self.samples[key]["signals"]
                ).type(dataType)
                self.samples[key]["labels"] = torch.tensor(
                    self.samples[key]["labels"]
                ).type(labelType)
        else:  # If it is not torch
            for key in self.samples.keys():
                self.samples[key]["signals"] = dataType(self.samples[key]["signals"])
                self.samples[key]["labels"] = labelType(self.samples[key]["labels"])
        ...
        if repr(self.dataType).find("torch") >= 0:
            for key in self.samples.keys():
                for secondKey in self.samples[key].keys():
                    self.samples[key][secondKey] = self.samples[key][secondKey].to(
                        device
                    )
...
class FacebookEgo:
    ...
    def loadData(self, filename, use234):
        ...
        if os.path.isfile(datasetFilename):
            ...
            existsRawData = os.path.isfile(
                os.path.join(self.dataDir, "facebook_combined.txt")
            )
            # And the zip file
            existsZipFile = os.path.isfile(
                os.path.join(self.dataDir, "facebook_combined.txt.gz")
            )
            if not existsRawData and not existsZipFile:  # We have to download it
                ...
                urllib.request.urlretrieve(
                    fbURL,
                    filename=os.path.join(self.dataDir, "facebook_combined.txt.gz"),
                )
                existsZipFile = True
            ...
            with open(datasetFilename, "wb") as datasetFile:
                pickle.dump({"adjacencyMatrix": self.adjacencyMatrix}, datasetFile)
...
class Authorship(_dataForClassification):
    ...
    def __init__(
        ...
        # Copy WANs
        self.selectedAuthor["train"]["WAN"] = thisAuthorData["WAN"][
            randPermTrain, :, :
        ].copy()
        self.selectedAuthor["valid"]["WAN"] = thisAuthorData["WAN"][
            randPermValid, :, :
        ].copy()
        self.selectedAuthor["test"]["WAN"] = thisAuthorData["WAN"][
            randPermTest, :, :
        ].copy()
        ...
        labelsTrain = np.concatenate(
            (np.ones(nTrainAuthor), np.zeros(nTrainRest)), axis=0
        )
        xValid = np.concatenate((xAuthorValid, xRestValid), axis=0)
        labelsValid = np.concatenate(
            (np.ones(nValidAuthor), np.zeros(nValidRest)), axis=0
        )
        xTest = np.concatenate((xAuthorTest, xRestTest), axis=0)
        labelsTest = np.concatenate((np.ones(nTestAuthor), np.zeros(nTestRest)), axis=0)
        ...
        if repr(dataType).find("torch") == -1:
            for key in self.selectedAuthor.keys():
                for secondKey in self.selectedAuthor[key].keys():
                    self.selectedAuthor[key][secondKey] = dataType(
            ...
            for key in self.selectedAuthor.keys():
                for secondKey in self.selectedAuthor[key].keys():
                    self.selectedAuthor[key][secondKey] = torch.tensor(
                        self.selectedAuthor[key][secondKey]
                    ).type(dataType)
    ...
    def to(self, device):
        ...
        if repr(self.dataType).find("torch") >= 0:
            ...
            for key in self.selectedAuthor.keys():
                for secondKey in self.selectedAuthor[key].keys():
                    self.selectedAuthor[key][secondKey] = self.selectedAuthor[key][
                        secondKey
                    ].to(device)
...
class MovieLens(_data):
    ...
    def __init__(
        ...
        if self.categorical:
            ...
            self.samples["train"]["labels"] = labelDataType(
                self.samples["train"]["labels"] - 1
            )
            self.samples["valid"]["labels"] = labelDataType(
                self.samples["valid"]["labels"] - 1
            )
            self.samples["test"]["labels"] = labelDataType(
                self.samples["test"]["labels"] - 1
            )
    ...
    def loadData(self, filename, *args):
        ...
        if os.path.isfile(datasetFilename):
            ...
            # or that we do not have nothing and we need to download it.
            existsRawData = os.path.isfile(
                os.path.join(dataDir, "ml-100k", "u.data")
            ...
            if not existsRawData and not existsZipFile:  # We have to download it
                ...
                urllib.request.urlretrieve(
                    mlURL, filename=os.path.join(dataDir, "ml-100k.zip")
                )
                existsZipFile = True
            if not existsRawData and existsZipFile:  # Unzip it
    ...
    def createGraph(self):
        ...
        subIndexDataPointsRestAll = np.nonzero(indexDataPoints[0] != self.labelID)[0]
        #   Nonzero points along the rows
        indexDataPointsRestAll = (
            indexDataPoints[0][subIndexDataPointsRestAll],
        ...
        allTrainPointsID = (
            labelIDpadding.astype(allTrainPointsID.dtype),
            allTrainPointsID,
        )
        ...
        nonzeroSqrtDiagonalIndex = (sqrtDiagonal > zeroTolerance).astype(
            sqrtDiagonal.dtype
        )
        #   Set the zero elements to 1
        sqrtDiagonal[sqrtDiagonal < zeroTolerance] = 1.0
        #   Invert safely
        ...
        #   division.
        normalizedMatrix[np.abs(normalizedMatrix) < zeroTolerance] = 0.0
    ...
    def astype(self, dataType):
        ...
        if repr(dataType).find("torch") == -1:
            ...
            self.incompleteMatrix = torch.tensor(self.incompleteMatrix).type(dataType)
            self.adjacencyMatrix = torch.tensor(self.adjacencyMatrix).type(dataType)
...
class TwentyNews(_dataForClassification):
    ...
    def getData(self, dataSubset):
        ...
        if dataSubset == "train":
            dataset.remove_short_documents(nwords=self.nWordsShortDocs, vocab="full")
            # Get rid of images
            dataset.remove_encoded_images()
            self.nClasses = len(dataset.class_names)
        else:
            dataset.remove_short_documents(
                nwords=self.nWordsShortDocs, vocab="selected"
            )
    ...
    def createValidationSet(self, ratio):
        ...
        self.samples["train"]["labels"] = self.samples["train"]["labels"][trainIndices]
        # Update the numbers
    ...
    def createGraph(self, *args):
        ...
        dist, idx = distance_sklearn_metrics(
            self.graphData, k=self.M, metric=self.distMetric
        )
    ...
    def reduceDataset(self, nTrain, nValid, nTest):
        ...
        if nValid < self.nValid:
            ...
            self.samples["valid"]["labels"] = self.samples["valid"]["labels"][
                validIndices
            ]
            self.nValid = nValid
        if nTest < self.nTest:
            ...
            self.samples["test"]["labels"] = self.samples["test"]["labels"][testIndices]
            self.nTest = nTest
