...
def computeSourceNodes(A, C):
    ...
    # Compute communities
    communityClusters = SpectralClustering(
        n_clusters=C, affinity="precomputed", assign_labels="discretize"
    )
            ...
            for n in range(N):  # Go through all nodes in order
                ...
                if len(inds_equallyfar) == 1:  # if a unique farthest node to
                    ...
                    W[
                        pind[
                            sortedIndices[
                                np.min(inds_equallyfar)
                                + r[0 : nn - np.min(inds_equallyfar) + 1]
                            ]
                        ],
                        n,
                    ] = 1
...
def permSpectralProxies(S):
    ...
    while len(nodes) < M:
        ...
        phi_eig, phi_ast_k = np.linalg.eig(STkSk[remainingNodes][:, remainingNodes])
...
class Graph:
    ...
    def __init__(self, graphType, N, graphOptions):
        ...
        self.selfLoops = (
            True if np.sum(np.abs(np.diag(self.W)) > zeroTolerance) > 0 else False
        )
        # \\\ Degree matrix:
